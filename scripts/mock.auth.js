...
(() => {
  ...
  function addHours(h) { return now() + h * 3600 * 1000; }
  ...
  function renderTx(lines, ccy) {
    if (!Array.isArray(lines) || !lines.length) {
      return `<div class="k">No transactions yet.</div>`;
    }
    const rows = lines
      .slice()
  ...
  // expose
  window.MFC_AUTH = {
    login, loadSession, clearSession, requireSession, getUser, fmtMoney, renderTx
  };
})();
